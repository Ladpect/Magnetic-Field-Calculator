...
class WindowClass(QMainWindow, form_class):
    ...
    def result(self):
        ...
        #-----------------------------------------------------------------------------------
        try:
            pos1 = float(self.pos1.text())
            pos2 = float(self.pos2.text())
            inten1 = float(self.inten1.text())
            inten2 = float(self.inten2.text())
            calpos = float(self.calpos.text())
            pm1 = 1
            pm2 = 1
            self.ErrorMsg.setText("")
            if pos1 > pos2:
                self.ErrorMsg.setText("도선 1의 위치값이 도선 2의 위치값보다 큽니다.")
                initialize(self)
                return 0
            if self.down1.isChecked() is False and self.up1.isChecked() is False:
                self.ErrorMsg.setText("도선 1의 전류의 방향이 정의되지 않았습니다.")
                initialize(self)
                return 0
            if self.down2.isChecked() is False and self.up2.isChecked() is False:
                self.ErrorMsg.setText("도선 2의 전류의 방향이 정의되지 않았습니다.")
                initialize(self)
                return 0
            if pos1 == calpos:
                self.ErrorMsg.setText("측정하려는 위치가 도선1의 위치와 같습니다.")
                initialize(self)
                return 0
            if pos2 == calpos:
                self.ErrorMsg.setText("측정하려는 위치가 도선2의 위치와 같습니다.")
                initialize(self)
                return 0
            Magby1 = inten1 / abs(pos1 - calpos)
            Magby2 = inten2 / abs(pos2 - calpos)
            ...
            if calpos > pos2: #도선 2의 위치값보다 측정 하려는 위치의 위치값이 [더 큰 경우]
                ...
                if self.down1.isChecked():
                    ...
                    pm2 = 1
            MagForce = Magby1 * pm1 + Magby2 * pm2
            if MagForce > 0:
